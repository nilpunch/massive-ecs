...
// 		private List<(Chunk chunkId, int size)> _activeChunks;
//
// 		[SetUp]
// 		public void SetUp()
// 		{
// 			_allocator = new Allocator();
// 			_activeChunks = new List<(Chunk chunkId, int size)>();
...
// 							Assert.GreaterOrEqual(chunkId.AsInt, 0);
...
// 		{
// 			var freelistCount = 0;
// 			foreach (var head in _allocator.FreeChunks)
// 			{
// 				var current = head;
// 				while (current != Allocator.ChunkEndId)
// 				{
// 					freelistCount++;
...
// 		{
// 			var usedFreeLists = _allocator.FreeChunks.Count(h => h != Allocator.ChunkEndId);
//
// 			var tailCount = 0;
// 			for (var i = 0; i < _allocator.ChunkCount; i++)
// 			{
// 				if (_allocator.Chunks[i].NextFreeId == ~Allocator.ChunkEndId)
// 					tailCount++;
// 			}
//
