...
namespace Massive
{
	...
	public readonly struct GroupView<T1, T2> : IView<T1, T2>
	{
			...
			switch (_group.IsOwning(_components1), _group.IsOwning(_components2))
			{
				...
				case (false, true):
					foreach (var (pageIndex, pageLength, indexOffset) in new PageSequence(data1.PageSize, groupIds.Length))
					{
						...
						{
							int id = groupIds[indexOffset + dense];
							invoker.Apply(id, ref _components1.Get(id), ref page2[dense]);
						}
					}
					break;
				...
				case (true, false):
					foreach (var (pageIndex, pageLength, indexOffset) in new PageSequence(data1.PageSize, groupIds.Length))
					{
						...
						{
							int id = groupIds[indexOffset + dense];
							invoker.Apply(id, ref page1[dense], ref _components2.Get(id));
						}
					}
					break;
				...
				case (false, false):
					for (int dense = groupIds.Length - 1; dense >= 0; dense--)
					{
						...
						invoker.Apply(id, ref _components1.Get(id), ref _components2.Get(id));
					}
					break;
			}
		}
